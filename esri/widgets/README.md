...
I'll submit an issue to maquette for this, but in the meantime, the classes can be split up:

```tsx
render() {
  // workaround: spilt up classes
  // CSS.example = "esri-example"
  // CSS.active = "esri-example--active"
  const dynamicClass = {
    [CSS.example]: this.someCondition,
    [CSS.active]: this.someCondition
  };
...
* [BasemapToggle](./examples/widgets/BasemapToggle.tsx)
* [Compass](./examples/widgets/Compass.tsx)
* [Zoom](./examples/widgets/Zoom.tsx)

### Patterns

You can take a look at the [patterns](https://devtopia.esri.com/WebGIS/arcgis-js-api/blob/4master/esri/widgets/examples/patterns) to look at some focused examples/snippets.

If your source files are being served, you can load the pattern page for an improved browsing experience (e.g., `http://<jsapi-4-url>/esri/widgets/examples/patterns`)

**Note:** IIS (Windows) requires adding `*.tsx` as a MIME type for patterns to load correctly.
